...
function startAgeUpdate() {
  ...
  ageUpdateInterval = setInterval(() => {
    ...
    let remainingMilliseconds = elapsedTime - years * 1000 * 60 * 60 * 24 * 365.25;

    const timeUnits = ["months", "days", "hours", "minutes", "seconds"];
    const values = timeUnits.map((unit) => {
      const value = Math.floor(remainingMilliseconds / timeUnitInMilliseconds[unit]);
      remainingMilliseconds -= value * timeUnitInMilliseconds[unit];
      return value;
    });

    result.innerText = (elapsedTime / 1000 / 60 / 60 / 24 / 365).toPrecision(11);
  }, 50);
}
